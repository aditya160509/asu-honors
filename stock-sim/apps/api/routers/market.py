...
def get_market(timeline_id: int = 1, db: Session = Depends(get_db)) -> MarketGridResponse:
    return market_service.get_market_grid(db, timeline_id)


@router.get("/market/cycle", response_model=CycleStateResponse)
def get_cycle(timeline_id: int = 1, db: Session = Depends(get_db)) -> CycleStateResponse:
    return market_service.get_cycle_state(db, timeline_id)


@router.get("/companies/{ticker}", response_model=CompanyDetail)
def get_company(ticker: str, timeline_id: int = 1, db: Session = Depends(get_db)) -> CompanyDetail:
...
def get_company_valuation(
    ticker: str, timeline_id: int = 1, db: Session = Depends(get_db)
) -> ValuationResponse:
