...
>
> Last synced with code: 2026-07-30 (final whole-branch review fixes for `_generate_concalls_for_quarter` in `engine/orchestrator.py`: the loop-wide `except Exception: return` now logs via `logger.warning(..., exc_info=True)` instead of swallowing silently now that Task 5's factor-score nudges/`NewsFeed` inserts share that savepoint; `moat_score`/the nudge block's `management_quality`/`moat_score` reads now guard the underlying column being `None` — defaulting to the neutral `50.0` midpoint already used elsewhere in this file — instead of raising `TypeError` from `float(None)`; and the per-company `previous_concall` query was replaced with a new batched `_load_previous_concalls` helper (one `IN (...)` query for all companies, modeled on `_load_concall_guidance_signal`/`_load_concall_extended_signals`). **No formula, math, or config-key changed** — this is a swallowed-exception-visibility fix, a null-guard fix, and a query-batching fix, none of which alter any price/value output. No section below needed updating.).
>
> Previously synced: 2026-07-30 (`_generate_concalls_for_quarter` in `engine/orchestrator.py` now passes the prior ConCall/BalanceSheet/CashFlowStatement rows into `generate_concall` for trend-chaining, and applies a small clamped structural nudge to the *current quarter's fresh* `CompanyFactorScore.management_quality`/`moat_score` from that call's `driver_deltas["guidance"]`/`["earnings_surprise"]` — scaled by `CONCALL_FACTOR_NUDGE_SCALE=6.0` and clamped to `±CONCALL_FACTOR_NUDGE_CLAMP=1.5` points, then re-clamped to `[0, 100]`. This is the same "nudge factor scores (structural, persistent) → feeds back into IV" path §1's mental-model diagram already describes for events/news; con-calls are now a second, much smaller-magnitude source feeding that same path. No existing formula/constant in §2–§6 changed, and no new config-parameter row was added (the two new constants live in `engine/orchestrator.py`, not `seed_config.py`) — nothing else below needed updating.).
>
> Previously synced: 2026-07-22 (Future Lab hardening pass: `engine/cycle.py`'s `advance_cycle_phase` now logs an error before its existing silent fallback to `"expansion"` when `current_phase` isn't recognized, instead of resetting silently. **No formula, math, or config-key changed** — the fallback's return value and trigger condition are unchanged, this only adds a `logger.error(...)` call on that already-existing path. No section below needed updating.).
>
> Previously synced: 2026-07-22 (SPEED.md Tier 1 output-safe performance pass touched `_refresh_fundamentals`/`_generate_fake_quarterly_financials`/`_execute_events`/`_load_tick_state` in `engine/orchestrator.py` and `db/timeline_resolver.py` — query batching and caching only (duplicate Timeline/SimulationState fetch removed, Company/Industry lookups batched, `timeline_chain` resolved once per tick, moving-average window query tightened, quarter-boundary idempotency checks batched into 3 `IN (...)` queries). **No formula, math, or config-key changed** — verified by a new pinned-seed golden-master regression test (`test_pinned_seed_snapshot_stable_across_optimizations` in `tests/test_orchestrator.py`) asserting byte-identical tick outputs before and after. No section below needed updating.).
>
> Previously synced: 2026-07-21 (`moat_composite` no longer raises `ZeroDivisionError` on empty subscores, matching `financial_quality_composite`'s existing empty-pillar fallback; `create_branch` now clones the parent timeline's `MoatSubscore`/`FinancialQualitySubscore` rows onto a new branch at fork time, closing the actual root cause — every Future Lab branch previously started with zero subscore rows and crashed on its first fast-forwarded quarter; see §2.3).
>
