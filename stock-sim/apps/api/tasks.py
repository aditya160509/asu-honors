"""In-process job functions for asynchronous Future Lab branch execution.

Each task opens its OWN DB session rather than reusing a request-scoped
session, since it runs outside the request lifecycle with no access to the
...
`_session_factory` is module-level and swappable so tests in eager/inline
mode can point these tasks at the
...
def run_fast_forward_job(timeline_id: int, target_days: int) -> dict:
    ...
    db = _session_factory()
    try:
        branch_service.extend_timeline(db, timeline_id, target_days, persist_progress=True)
        db.commit()
        ...
        try:
            ...
            if timeline is not None:
                ...
                timeline.recovery_action = "Review the engine error, correct the scenario inputs, then duplicate or rerun the branch."
...
def run_ensemble_member_job(timeline_id: int, target_days: int) -> dict:
    ...
    independently from single-branch fast-forwards in logs and diagnostics.
    """
    return run_fast_forward_job(timeline_id, target_days)
