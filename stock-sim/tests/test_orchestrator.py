...
def _seed_minimal(session: Session) -> int:
    ...
    session.execute(sa_text("""INSERT INTO config_parameters (key, value, scope, created_at, updated_at) VALUES ('beta_pe', '0.5', 'global', datetime('now'), datetime('now'))"""))
    session.execute(sa_text("""INSERT INTO config_parameters (key, value, scope, created_at, updated_at) VALUES ('beta_g', '0.3', 'global', datetime('now'), datetime('now'))"""))
