...
export function RowExpandedContent({ row }: { row: EnrichedCompany }) {
  ...
  const sparkColor = row.ivGapPct != null && row.ivGapPct < -3
    ? cssVar("--positive") || "#22c55e"
    : row.ivGapPct != null && row.ivGapPct > 3
      ? cssVar("--negative") || "#ef4444"
  ...
  return (
    <div className="flex h-full items-stretch border-t border-border/40 bg-bg-tertiary/40">
      ...
      <div className="flex items-center gap-4 overflow-x-auto px-3 scrollbar-none">
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-micro text-text-tertiary uppercase tracking-wider">Sector</span>
          <span className="text-small text-text-secondary normal-case truncate max-w-[140px]">{row.industry_name}</span>
        </div>
      </div>
    </div>
  );
